...
# DB Subnet Group for DR RDS
module "db_subnet_group" {
  source       = "../modules/rds-subnet-group"
  name         = "${local.name_prefix}-db-subnets"
  subnet_ids   = [
    module.subnet_private_az_1.subnet_id,
    module.subnet_private_az_2.subnet_id
  ]
  description  = "Subnet group for Single-AZ RDS in secondary region"
}
